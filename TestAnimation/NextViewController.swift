...
//        println(arrSorted)
        ...
        var a = NSArray(array: [111,222,333])
        a.objectAtIndex(0)
...
//            self.mSubView!.hidden =  true
    ...
    func animateViewAndLayerTogether() {
        UIView.animateWithDuration(1.0, delay: 0.0, options: UIViewAnimationOptions.CurveLinear, animations: { () -> Void in
            ...
            var angle = CGFloat(180.0 * M_PI / 180.0)
            
            var xform = CGAffineTransformMakeRotation(angle)
            self.firstView.transform = xform
            
            var layerAnimate = CABasicAnimation(keyPath: "transform")
