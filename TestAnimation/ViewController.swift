...
class ViewController: UIViewController{
    ...
    @IBAction func scrolled(sender: AnyObject) {
        var scroll : UISlider = sender as! UISlider
    ...
    override func viewDidLoad() {
        ...
        var btn = UIButton(frame: CGRect(x: 10, y: 30, width: 50, height: 20))
        ...
        var btn1 = UIButton(frame: CGRect(x: 100, y: 30, width: 50, height: 20))
        btn1.titleLabel?.text = "click"
        btn1.addTarget(self, action: "on2Click", forControlEvents: UIControlEvents.TouchUpInside)
        btn1.backgroundColor = UIColor.darkGrayColor()
        
        self.view.addSubview(btn1)
        
        var btn3 = UIButton(frame: CGRect(x: 190, y: 30, width: 50, height: 20))
        btn3.titleLabel?.text = "click"
        btn3.addTarget(self, action: "on3Click", forControlEvents: UIControlEvents.TouchUpInside)
...
//        }
    ...
    func on3Click() {
        UIView.transitionWithView(self.view1!, duration: 2, options: UIViewAnimationOptions.CurveEaseIn, animations: { () -> Void in
            ...
            println("Finished:" + (finished ? "yes" : "no"))
        }
        
        var arr : NSMutableArray = [0,1,2,3]
        var index = arr.indexOfObject(3)
        println("\(index)")
    }
    ...
    override func animationDidStop(anim: CAAnimation!, finished flag: Bool) {
        println(anim.description + " has been finished")
    }
    
    override func animationDidStart(anim: CAAnimation!) {
        println(anim.description + " did start")
    }
}
